// ============================================================
// pages/OrderSuccessPage.jsx — Trang hoàn tất đơn hàng
//
// Đọc thông tin đơn từ localStorage để hiển thị
// Sau khi hiển thị xong → xóa localStorage (không cần nữa)
// ============================================================
...
export default function OrderSuccessPage() {
  ...
  useEffect(() => {
    ...
    if (!saved) {
      navigate("/", { replace: true }); // không có đơn → về trang chủ
      return;
    }
    setOrder(saved);
    clearOrderFromStorage(); // xóa sau khi đã hiển thị
  }, [navigate]);
            ...
            <div className="card-body p-4 p-md-5">
              ...
              <div className="text-start mb-4">
                ...
                <div className="small text-muted d-flex flex-column gap-2">
                  <div><i className="bi bi-person me-2"></i><strong>{order.customer.fullName}</strong></div>
                  <div><i className="bi bi-telephone me-2"></i>{order.customer.phone}</div>
                  <div><i className="bi bi-house me-2"></i>{order.customer.address}</div>
                  {order.customer.note && (
                    <div><i className="bi bi-chat-left-text me-2"></i><em>"{order.customer.note}"</em></div>
                  )}
                </div>
              </div>
              ...
              <div className="text-start mb-4">
                ...
                {order.items.map((item) => (
                  <div key={item.id} className="d-flex justify-content-between small text-muted mb-1">
                    ...
                    <span className="fw-semibold text-dark">{formatPrice(item.price * item.qty)}</span>
                  </div>
                ))}
                <hr />
              ...
              <div className="rounded-3 p-3 mb-4 text-start small" style={{ background: "#eff6ff" }}>
              ...
              <div className="d-grid gap-2">
                <button className="btn btn-primary fw-bold" onClick={() => navigate("/products")}>
